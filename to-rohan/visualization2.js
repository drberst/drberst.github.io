...
const DEBUG_AREA_HEIGHT = 0; // Height of the debug area
...
function drawWaveform() {
    ...
    // const baseHeight = 1080 / 4;
    const baseHeight = 250;
    const spacing = 100;
    // Set the waveform drawing area dimensions
    let waveformWidth = WIDTH; // Adjust width for the waveform area
    let waveformHeight = 0; // Height of the waveform area
    let waveformX = 0; // Start drawing from the right side
    let waveformY = 0; // Start at the top
    // Draw the background of the graph
    // ctx.fillStyle = "hsl(0, 0%, 40%)"; // Semi-transparent background
    ...
    waveformWidth = WIDTH; // Adjust width for the waveform area
    waveformHeight = baseHeight; // Height of the waveform area
    waveformX = 0; // Start drawing from the right side
    waveformY = baseHeight; // Start at the top
    for (let i = 0; i < waveformArray.length; i++) {
        const x = waveformX + i * sliceWidth;
        const y = waveformY + (waveformArray[i] / 255.0) * waveformHeight;
    ...
    waveformWidth = WIDTH; // Adjust width for the waveform area
    waveformHeight = baseHeight * 2; // Height of the waveform area
    waveformX = 0; // Start drawing from the right side
    waveformY = baseHeight; // Start at the top
    for (let i = 0; i < waveformArray.length; i++) {
        const x = waveformX + i * sliceWidth;
    ...
    waveformWidth = WIDTH; // Adjust width for the waveform area
    waveformHeight = baseHeight * 3; // Height of the waveform area
    waveformX = 0; // Start drawing from the right side
    waveformY = baseHeight; // Start at the top
    for (let i = 0; i < waveformArray.length; i++) {
        const x = waveformX + i * sliceWidth;
